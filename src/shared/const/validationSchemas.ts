  ...
  .object({
    username: z
      ...
      .regex(USERNAME_PATTERN, {
        message:
          "Username must contain only latin letters, numbers, underscores, or hyphens.",
      })
      .min(1, { message: "This field has to be filled." })
      .min(6, { message: "Username must be at least 6 characters" })
      .max(30, {
        message: "The field must not contain more than 30 characters",
      }),
    email: z
      .string()
      .min(1, { message: "This field has to be filled." })
    ...
    password: z
      ...
      .max(20, {
        message: "The field must not contain more than 20 characters",
      }),
    confirmPassword: z.string(),
    isAgree: z
      .boolean()
      .refine((val) => val, { message: "The checkbox must be" + " checked" }),
  })
  .refine(
    (values) => {
...
export const changeProfileSchema = z.object({
  ...
  dateOfBirth: z
    ...
    .refine(
      (date) => {
        if (!date) return true;
        return (
          new Date().getTime() - date.getTime() > 13 * 365 * 24 * 60 * 60 * 1000
        );
      },
      {
  ...
  about: z
    ...
    .refine((value) => /^[0-9A-Za-zА-Яа-я\s\-_.'":,!]*$/.test(value), {
      message:
        "Allowed characters: 0-9, A-Z, a-z, А-Я, а-я, and special characters",
    })
    .optional(),
});
